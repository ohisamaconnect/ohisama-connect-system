#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
OC-OS Local Transcription Pilot v0.4.0

Principles:
- Coverage first: primary ASR must prefer capturing all spoken content.
- Do NOT inject the HHA glossary into primary decoding.
- Preserve faster-whisper RAW output unchanged.
- CLEAN preserves RAW boundaries except for very large music/silence gaps.
- Run Coverage Audit against the normalized speech stem so audible regions
  that were not meaningfully transcribed are flagged for review.
- Proper-noun correction belongs to a later HHA-grounded post-processing layer.

Input:
  UVR vocal stem / speech-focused audio.

Output:
  *_16k_mono.wav
  *_RAW.txt
  *_CLEAN.txt
  *_CLEAN.srt
  *_CLEAN.vtt
  *_AUDIT.txt
  *_TRANSCRIPT.json
"""

from __future__ import annotations

import argparse
import glob
import json
import math
import os
import re
import shutil
import site
import subprocess
import sys
import time
import wave
from pathlib import Path

VERSION = "0.4.0"
MINIMAL_PROMPT = "おひさまコネクト。日向坂46。あさくらじゅん。"
TERMINAL_PUNCTUATION = "。！？!?…"


def configure_windows_gpu_dlls() -> list[str]:
    if os.name != "nt":
        return []

    candidates: list[str] = []
    cuda_path = os.environ.get("CUDA_PATH")
    if cuda_path:
        candidates.append(str(Path(cuda_path) / "bin"))

    try:
        site_dirs = site.getsitepackages()
    except Exception:
        site_dirs = []

    for root in site_dirs:
        candidates.extend(glob.glob(str(Path(root) / "nvidia" / "cudnn" / "bin")))

    added: list[str] = []
    seen = set()

    for raw in candidates:
        path = str(Path(raw).resolve())
        key = path.casefold()
        if key in seen or not Path(path).is_dir():
            continue
        seen.add(key)

        try:
            os.add_dll_directory(path)
        except (AttributeError, FileNotFoundError, OSError):
            pass

        os.environ["PATH"] = path + os.pathsep + os.environ.get("PATH", "")
        added.append(path)

    return added


def format_clock(seconds: float, sep: str = ".") -> str:
    seconds = max(0.0, seconds)
    ms_total = int(round(seconds * 1000))
    hours, rem = divmod(ms_total, 3_600_000)
    minutes, rem = divmod(rem, 60_000)
    secs, ms = divmod(rem, 1000)
    return f"{hours:02d}:{minutes:02d}:{secs:02d}{sep}{ms:03d}"


def normalize_audio(input_path: Path, output_path: Path) -> Path:
    ffmpeg = shutil.which("ffmpeg")
    if not ffmpeg:
        raise RuntimeError("ffmpeg が PATH にありません。")

    cmd = [
        ffmpeg,
        "-hide_banner",
        "-loglevel", "warning",
        "-y",
        "-i", str(input_path),
        "-vn",
        "-ac", "1",
        "-ar", "16000",
        "-c:a", "pcm_s16le",
        str(output_path),
    ]

    print("[1/5] 16 kHz mono PCM16 へ変換中...")
    subprocess.run(cmd, check=True)
    return output_path


def normalize_readable_text(text: str) -> str:
    return re.sub(r"[ \t\r\n]+", " ", str(text or "")).strip()


def finalize_clean_text(text: str) -> str:
    text = normalize_readable_text(text)
    if text and text[-1] not in TERMINAL_PUNCTUATION:
        text += "。"
    return text


def valid_words(seg: dict) -> list[dict]:
    out = []
    for w in seg.get("words", []) or []:
        if w.get("start") is None or w.get("end") is None:
            continue
        if not str(w.get("word", "")):
            continue
        out.append(w)
    return out


def max_internal_gap(words: list[dict]) -> float:
    if len(words) < 2:
        return 0.0
    return max(
        max(0.0, float(b["start"]) - float(a["end"]))
        for a, b in zip(words, words[1:])
    )


def split_words_on_large_gap(words: list[dict], threshold: float) -> list[list[dict]]:
    if not words:
        return []
    groups: list[list[dict]] = [[words[0]]]
    for previous, current in zip(words, words[1:]):
        gap = max(0.0, float(current["start"]) - float(previous["end"]))
        if gap >= threshold:
            groups.append([])
        groups[-1].append(current)
    return [g for g in groups if g]


def build_clean_segments(raw_segments: list[dict], music_gap: float) -> list[dict]:
    clean: list[dict] = []

    for raw in raw_segments:
        raw_text = normalize_readable_text(raw.get("text", ""))
        if not raw_text:
            continue

        words = valid_words(raw)
        biggest_gap = max_internal_gap(words)

        if biggest_gap < music_gap or len(words) < 2:
            clean.append({
                "id": len(clean),
                "start": float(raw["start"]),
                "end": float(raw["end"]),
                "text": raw_text,
                "source": "raw-preserved",
                "max_internal_gap": biggest_gap,
            })
            continue

        for group in split_words_on_large_gap(words, music_gap):
            text = finalize_clean_text("".join(str(w["word"]) for w in group))
            if not text:
                continue
            clean.append({
                "id": len(clean),
                "start": float(group[0]["start"]),
                "end": float(group[-1]["end"]),
                "text": text,
                "source": "music-gap-split",
                "max_internal_gap": biggest_gap,
            })

    return clean


def write_timestamped_txt(path: Path, segments: list[dict]) -> None:
    lines = [
        f"[{format_clock(seg['start'])} --> {format_clock(seg['end'])}] {seg['text'].strip()}"
        for seg in segments
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def write_srt(path: Path, segments: list[dict]) -> None:
    blocks = []
    for i, seg in enumerate(segments, 1):
        blocks.append(
            f"{i}\n"
            f"{format_clock(seg['start'], ',')} --> {format_clock(seg['end'], ',')}\n"
            f"{seg['text'].strip()}\n"
        )
    path.write_text("\n".join(blocks), encoding="utf-8")


def write_vtt(path: Path, segments: list[dict]) -> None:
    blocks = ["WEBVTT\n"]
    for seg in segments:
        blocks.append(
            f"{format_clock(seg['start'])} --> {format_clock(seg['end'])}\n"
            f"{seg['text'].strip()}\n"
        )
    path.write_text("\n".join(blocks), encoding="utf-8")


def wav_rms_frames(path: Path, frame_seconds: float = 1.0) -> tuple[int, list[dict]]:
    with wave.open(str(path), "rb") as wf:
        sample_rate = wf.getframerate()
        channels = wf.getnchannels()
        width = wf.getsampwidth()
        if channels != 1 or width != 2:
            raise RuntimeError("Coverage Audit requires 16-bit mono WAV.")

        frame_count = max(1, int(sample_rate * frame_seconds))
        frames = []
        index = 0

        while True:
            raw = wf.readframes(frame_count)
            if not raw:
                break

            # int16 little-endian, no numpy dependency required.
            count = len(raw) // 2
            if count == 0:
                break

            sum_sq = 0.0
            peak = 0
            for i in range(0, len(raw) - 1, 2):
                value = int.from_bytes(raw[i:i+2], byteorder="little", signed=True)
                peak = max(peak, abs(value))
                f = value / 32768.0
                sum_sq += f * f

            rms = math.sqrt(sum_sq / count) if count else 0.0
            db = 20.0 * math.log10(max(rms, 1e-12))
            start = index * frame_seconds
            end = start + count / sample_rate
            frames.append({
                "start": start,
                "end": end,
                "rms_dbfs": db,
                "peak": peak / 32768.0,
            })
            index += 1

    return sample_rate, frames


def meaningful_word_intervals(raw_segments: list[dict], max_word_duration: float) -> list[tuple[float, float]]:
    intervals = []
    for seg in raw_segments:
        for w in valid_words(seg):
            start = float(w["start"])
            end = float(w["end"])
            duration = max(0.0, end - start)
            if duration <= max_word_duration:
                intervals.append((start, end))
    return intervals


def overlaps_any(start: float, end: float, intervals: list[tuple[float, float]]) -> bool:
    for a, b in intervals:
        if b <= start:
            continue
        if a >= end:
            continue
        return True
    return False


def group_uncovered_active_frames(
    frames: list[dict],
    word_intervals: list[tuple[float, float]],
    active_dbfs: float,
    min_gap_seconds: float,
) -> list[dict]:
    flagged = []
    current = None

    for frame in frames:
        active = frame["rms_dbfs"] >= active_dbfs
        covered = overlaps_any(frame["start"], frame["end"], word_intervals)
        bad = active and not covered

        if bad:
            if current is None:
                current = {
                    "start": frame["start"],
                    "end": frame["end"],
                    "max_rms_dbfs": frame["rms_dbfs"],
                }
            else:
                current["end"] = frame["end"]
                current["max_rms_dbfs"] = max(current["max_rms_dbfs"], frame["rms_dbfs"])
        elif current is not None:
            if current["end"] - current["start"] >= min_gap_seconds:
                flagged.append(current)
            current = None

    if current is not None and current["end"] - current["start"] >= min_gap_seconds:
        flagged.append(current)

    return flagged


def suspicious_segments(raw_segments: list[dict], max_word_duration: float) -> list[dict]:
    warnings = []
    for seg in raw_segments:
        duration = max(0.0, float(seg["end"]) - float(seg["start"]))
        text = normalize_readable_text(seg.get("text", ""))
        words = valid_words(seg)
        longest_word = max(
            [max(0.0, float(w["end"]) - float(w["start"])) for w in words] or [0.0]
        )
        chars_per_second = len(text) / duration if duration > 0 else 999.0

        reasons = []
        if longest_word > max_word_duration:
            reasons.append(f"word timestamp spans {longest_word:.1f}s")
        if duration >= 8.0 and chars_per_second < 0.8:
            reasons.append(f"very low text density ({chars_per_second:.2f} chars/s)")

        if reasons:
            warnings.append({
                "start": float(seg["start"]),
                "end": float(seg["end"]),
                "text": text,
                "reasons": reasons,
            })

    return warnings


def build_audit(
    normalized_wav: Path,
    raw_segments: list[dict],
    active_dbfs: float,
    uncovered_min_seconds: float,
    max_word_duration: float,
) -> dict:
    sample_rate, frames = wav_rms_frames(normalized_wav)
    intervals = meaningful_word_intervals(raw_segments, max_word_duration)
    uncovered = group_uncovered_active_frames(
        frames,
        intervals,
        active_dbfs,
        uncovered_min_seconds,
    )
    suspicious = suspicious_segments(raw_segments, max_word_duration)

    return {
        "sample_rate": sample_rate,
        "active_dbfs_threshold": active_dbfs,
        "uncovered_min_seconds": uncovered_min_seconds,
        "max_word_duration_seconds": max_word_duration,
        "uncovered_active_audio": uncovered,
        "suspicious_segments": suspicious,
    }


def write_audit(path: Path, audit: dict) -> None:
    lines = [
        "OC-OS TRANSCRIPTION COVERAGE AUDIT",
        f"active threshold: {audit['active_dbfs_threshold']:.1f} dBFS",
        "",
        "[Audible but not meaningfully covered by ASR words]",
    ]

    gaps = audit["uncovered_active_audio"]
    if not gaps:
        lines.append("none")
    else:
        for item in gaps:
            lines.append(
                f"{format_clock(item['start'])} --> {format_clock(item['end'])} "
                f"max_rms={item['max_rms_dbfs']:.1f} dBFS"
            )

    lines.extend(["", "[Suspicious ASR segments]"])
    suspicious = audit["suspicious_segments"]
    if not suspicious:
        lines.append("none")
    else:
        for item in suspicious:
            lines.append(
                f"{format_clock(item['start'])} --> {format_clock(item['end'])} "
                f"{' / '.join(item['reasons'])} :: {item['text']}"
            )

    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def main() -> int:
    parser = argparse.ArgumentParser(description="OC-OS Local Transcription Pilot v0.4.0")
    parser.add_argument("input", type=Path)
    parser.add_argument("--output-dir", type=Path, default=None)
    parser.add_argument("--model", default="large-v3")
    parser.add_argument("--device", choices=["cuda", "cpu"], default="cuda")
    parser.add_argument("--compute-type", default=None)
    parser.add_argument("--language", default="ja")
    parser.add_argument("--beam-size", type=int, default=5)
    parser.add_argument("--music-gap", type=float, default=30.0)
    parser.add_argument("--active-dbfs", type=float, default=-35.0)
    parser.add_argument("--uncovered-min-seconds", type=float, default=2.0)
    parser.add_argument("--max-word-duration", type=float, default=3.0)
    parser.add_argument("--no-normalize", action="store_true")
    args = parser.parse_args()

    input_path = args.input.expanduser().resolve()
    if not input_path.exists():
        print(f"ERROR: input not found: {input_path}", file=sys.stderr)
        return 2

    output_dir = args.output_dir.expanduser().resolve() if args.output_dir else input_path.parent
    output_dir.mkdir(parents=True, exist_ok=True)

    compute_type = args.compute_type or ("int8_float16" if args.device == "cuda" else "int8")
    stem = input_path.stem
    audio_for_asr = input_path

    if not args.no_normalize:
        audio_for_asr = normalize_audio(input_path, output_dir / f"{stem}_16k_mono.wav")
    else:
        print("[1/5] 音声変換を省略します。")
        if input_path.suffix.lower() != ".wav":
            raise RuntimeError("Coverage Audit with --no-normalize requires a 16-bit mono WAV input.")

    dll_dirs = configure_windows_gpu_dlls()
    if dll_dirs:
        print("      GPU DLL dirs:")
        for d in dll_dirs:
            print(f"        - {d}")

    from faster_whisper import WhisperModel

    print(
        f"[2/5] version={VERSION} model={args.model} device={args.device} "
        f"compute_type={compute_type} language={args.language}"
    )
    print("      primary ASR mode=coverage-first / glossary injection=OFF")

    model = WhisperModel(args.model, device=args.device, compute_type=compute_type)

    started = time.time()
    segments_gen, info = model.transcribe(
        str(audio_for_asr),
        language=args.language,
        task="transcribe",
        beam_size=args.beam_size,
        vad_filter=True,
        vad_parameters=dict(min_silence_duration_ms=500, speech_pad_ms=250),
        condition_on_previous_text=False,
        initial_prompt=MINIMAL_PROMPT,
        hotwords=None,
        word_timestamps=True,
    )

    raw_segments: list[dict] = []
    for seg in segments_gen:
        raw_segments.append({
            "id": int(seg.id),
            "start": float(seg.start),
            "end": float(seg.end),
            "text": seg.text.strip(),
            "avg_logprob": float(seg.avg_logprob) if seg.avg_logprob is not None else None,
            "no_speech_prob": float(seg.no_speech_prob) if seg.no_speech_prob is not None else None,
            "words": [
                {
                    "start": float(w.start),
                    "end": float(w.end),
                    "word": w.word,
                    "probability": float(w.probability) if w.probability is not None else None,
                }
                for w in (seg.words or [])
                if w.start is not None and w.end is not None
            ],
        })

    elapsed = time.time() - started

    print("[3/5] RAWを尊重し、楽曲またぎだけHybrid分割中...")
    clean_segments = build_clean_segments(raw_segments, args.music_gap)

    print("[4/5] Coverage Audit実行中...")
    audit = build_audit(
        Path(audio_for_asr),
        raw_segments,
        args.active_dbfs,
        args.uncovered_min_seconds,
        args.max_word_duration,
    )

    base = output_dir / stem
    raw_txt = Path(str(base) + "_RAW.txt")
    clean_txt = Path(str(base) + "_CLEAN.txt")
    clean_srt = Path(str(base) + "_CLEAN.srt")
    clean_vtt = Path(str(base) + "_CLEAN.vtt")
    audit_txt = Path(str(base) + "_AUDIT.txt")
    transcript_json = Path(str(base) + "_TRANSCRIPT.json")

    write_timestamped_txt(raw_txt, raw_segments)
    write_timestamped_txt(clean_txt, clean_segments)
    write_srt(clean_srt, clean_segments)
    write_vtt(clean_vtt, clean_segments)
    write_audit(audit_txt, audit)

    payload = {
        "pipeline": "OC-OS Local Transcription Pilot",
        "version": VERSION,
        "input": str(input_path),
        "audio_for_asr": str(audio_for_asr),
        "model": args.model,
        "device": args.device,
        "compute_type": compute_type,
        "language_requested": args.language,
        "language_detected": getattr(info, "language", None),
        "language_probability": getattr(info, "language_probability", None),
        "duration": getattr(info, "duration", None),
        "duration_after_vad": getattr(info, "duration_after_vad", None),
        "elapsed_seconds": elapsed,
        "recognition": {
            "mode": "coverage-first",
            "initial_prompt": MINIMAL_PROMPT,
            "hotwords": None,
        },
        "cleaning": {
            "mode": "hybrid-raw-preserving",
            "music_gap_seconds": args.music_gap,
        },
        "coverage_audit": audit,
        "raw_segments": raw_segments,
        "clean_segments": clean_segments,
    }

    transcript_json.write_text(
        json.dumps(payload, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )

    print("[5/5] 完了")
    print(f"elapsed: {elapsed:.1f} sec")
    print(f"RAW  : {raw_txt}")
    print(f"CLEAN: {clean_txt}")
    print(f"AUDIT: {audit_txt}")
    print(f"JSON : {transcript_json}")

    print("\n----- COVERAGE AUDIT -----")
    if not audit["uncovered_active_audio"] and not audit["suspicious_segments"]:
        print("warnings: none")
    else:
        for item in audit["uncovered_active_audio"]:
            print(
                f"UNCOVERED {format_clock(item['start'])} --> {format_clock(item['end'])} "
                f"max_rms={item['max_rms_dbfs']:.1f} dBFS"
            )
        for item in audit["suspicious_segments"]:
            print(
                f"SUSPICIOUS {format_clock(item['start'])} --> {format_clock(item['end'])} "
                f"{' / '.join(item['reasons'])}"
            )

    print("\n----- CLEAN PREVIEW -----")
    for seg in clean_segments[:40]:
        print(
            f"[{format_clock(seg['start'])} --> {format_clock(seg['end'])}] "
            f"{seg['text']}"
        )
    if len(clean_segments) > 40:
        print(f"... ({len(clean_segments) - 40} more segments)")

    return 0


if __name__ == "__main__":
    raise SystemExit(main())
